...
class UserServiceTest extends FlatSpec with MustMatchers {
  ...
  it must "delete a user" in {
    Await.result(userServiceImpl.deleteFun(user4.id), 1 second) must be(user4)
    val deleteAgain = Try(Await.result(userServiceImpl.deleteFun(user4.id), 1 second))
